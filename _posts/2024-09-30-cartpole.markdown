...
\displaylines{
   ...
   \\
   \\ \frac {F_u\cos{\theta} + mL\ddot{\theta}\cos^2{\theta} - mL\dot{\theta}\sin^2{\theta}{\cos\theta} - (M+m)g\sin\theta} {L(M+m)} = \ddot{\theta}
   \\ \frac {-F_u + (M+m)g\theta} {L(M+m)} \approx \ddot{\theta}
}
$$
...
So we can see here, that if any of our eigenvalues are positive, D will have a position value in it, and e^{Dt} will explode to massive values over time. This is why stable systems require that all of the eigenvalues are negative. The video linked above also shows that when transformed to discrete time, all the eigenvalues must be less than 1 for the system to be stable.
